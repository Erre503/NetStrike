...
class MainInterfaccia(ctk.CTkFrame):
    ...
    def initUI(self):
        ...
        self.finestraPrincipale.wm_iconbitmap(icona)
    ...
    def selezionaPlugin(self, name):
        if self.labelPSelezionabili.cget("text") == "AVAILABLE PLUG-IN":
            ...
            self.plugin_selezionato = self.plugin_files.get(name)['id']
            self.labelInfoPluginSelezionato.configure(text=f"SELECTED LOG: {name}")
            self.coreApplicazione.ottieni_dettagli_plugin(self.plugin_selezionato)
    ...
    def modificaPlugin(self):
        ...
        finestraEdit.geometry("450x300")
        icona = os.path.join(os.path.dirname(__file__), 'er.ico')
        finestraEdit.wm_iconbitmap(icona)
    ...
    def configuraTest(self):
        ...
        icona = os.path.join(os.path.dirname(__file__), 'er.ico')
        finestraConfig.wm_iconbitmap(icona)
    ...
    def cambiaView(self):
        if self.labelPSelezionabili.cget("text") == "AVAILABLE PLUG-IN":
            ...
            self.bottoneRinominaP.pack(pady=10)
            self.bottoneView.configure(text="VIEW LOGs")
