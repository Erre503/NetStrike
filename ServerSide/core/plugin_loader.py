...
def caricaPlugin(req, nome_plugin):  # folder e req si possono dichiarare come variabili globale se necessario
    # (il diagramma dice di passargli solo nome_plugin)

    for file in os.listdir('../plugins'):  # il for verifica tutti i file all'interno della cartella
        if nome_plugin == file[:-3] and file.endswith(
                '.py'):  # verifica se il file e' python e se corrisponde a quello cercato,altrimenti passa al prossimo
...
if (__name__ == "__main__"):
    ...
    folder = Path(
        __file__).resolve().parent.parent / "plugins"  # assegna il percorso della cartella basandosi su quello del plugin loader
    ...
    plugin = caricaPlugin(folder, req,
                          nome_plugin)  # viene caricato il modulo del file richiesto in una variabile per poter essere eseguita
