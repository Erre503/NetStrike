...
def caricaPlugin(folder, req, nome_plugin):
    ...
    for file in os.listdir(folder):
        if nome_plugin == file[:-3] and file.endswith('.py'):
            ...
            modulo = importlib.import_module(nome_plugin) #contenuto del file (plug in) py
...
if(__name__ == "__main__"):
    ...
    nome_plugin = input()
    req = "execute"
    folder = Path(__file__).resolve().parent.parent / "plugins"  # Una cartella sopra quella corrente
