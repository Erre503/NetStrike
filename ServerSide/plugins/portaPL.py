...
class Plugin(Interfaccia_Plugin):
    ...
    def execute(cls):
        ...
        print("Porte aperte: " + porteAperte)
...
def scan_tcp(ip, porta, timeout):
    try:
        ...
        if result == 0:
            return "Porta " + porta + " aperta/n"  # se e' 0 la connessione con la porta e' riuscita
        else:
            return "Porta " + porta + " chiusa/n"
    except socket.error:
        return "Errore di connessione alla porta " + porta
    finally:
...
def scan_udp(ip, porta, timeout):
    try:
        ...
        return "Porta " + porta + " aperta/n"
    except socket.error:
        return "Errore di connessione alla porta " + porta + " (probabilmente chiusa)" #in caso scada il time out si da per chiusa la porta
    finally:
        sock.close()  
